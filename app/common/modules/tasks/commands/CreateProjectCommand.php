...
class CreateProjectCommand implements CommandInterface
{
    ...
    {
        return $this->matchTextArgument($text, 'create\s+project') !== null;
    }

    public function parse(string $text): array
    {
        return ['name' => $this->matchTextArgument($text, 'create\s+project') ?? ''];
    }
    ...
    public function execute(array $payload, array $context = []): CommandExecutionResult
    {
        ...
        if ($project === null) {
            return new CommandExecutionResult(true, $this->name(), 'Project creation failed', $payload);
        }

        return new CommandExecutionResult(true, $this->name(), 'Project created: #' . $project->id, $payload);
    }
}
